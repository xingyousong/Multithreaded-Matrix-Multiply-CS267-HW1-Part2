...
static double* weird_transformation(double* src, int lda, int stride) {
  ...
  int rowNums = (lda / stride);
  if (lda % stride == 0)
    rowNums = lda / stride;
  else
    rowNums = lda / stride + 1;
  ...
  #pragma omp parallel num_threads(2)
  {
    ...
    for (int i = 0; i <= M - STRIDE; i += STRIDE){
      for (int j = 0; j <= N - 4; j += 4){
          ...
          //loadu cols
          c_col_0 = _mm256_loadu_pd(Cij);
          c_col_1 = _mm256_loadu_pd(Cij + lda);
          c_col_2 = _mm256_loadu_pd(Cij + 2*lda);
          c_col_3 = _mm256_loadu_pd(Cij + 3*lda);
          c_col_4 = _mm256_loadu_pd(Cij + 4);
          c_col_5 = _mm256_loadu_pd(Cij + lda + 4);
          c_col_6 = _mm256_loadu_pd(Cij + 2*lda + 4);
          c_col_7 = _mm256_loadu_pd(Cij + 3*lda + 4);

          __m256d a_row_k_first_half;
          __m256d a_row_k_second_half;
          for (int k = 0; k < K; ++k){
            a_row_k_first_half = _mm256_loadu_pd(A+weird_offset(i,k,lda,STRIDE));
            a_row_k_second_half = _mm256_loadu_pd(A+weird_offset(i,k,lda,STRIDE)+ 4);
          ...
          _mm256_storeu_pd(Cij,         c_col_0);
          _mm256_storeu_pd(Cij+lda,     c_col_1);
          _mm256_storeu_pd(Cij+2*lda,   c_col_2);
          _mm256_storeu_pd(Cij+3*lda,   c_col_3);
          _mm256_storeu_pd(Cij+4,       c_col_4);
          _mm256_storeu_pd(Cij+lda+4,   c_col_5);
          _mm256_storeu_pd(Cij+2*lda+4, c_col_6);
          _mm256_storeu_pd(Cij+3*lda+4, c_col_7);
      }

      //leftover//
      for (int j = (N/4)*4; j < N; j++){
          c_col_0 = _mm256_loadu_pd(C + i + j*lda);
          c_col_1 = _mm256_loadu_pd(C + i + j*lda + 4);
          __m256d a_row_k_first_half;
          __m256d a_row_k_second_half;
          for (int k = 0; k < K; k++){
            a_row_k_first_half  = _mm256_loadu_pd(A+weird_offset(i,k,lda,STRIDE));
          ...
          _mm256_storeu_pd(C+i+j*lda,      c_col_0);
          _mm256_storeu_pd(C+i+j*lda + 4,  c_col_1);
      }
    }
  }
}
  ...
  #pragma omp parallel num_threads(1)
  {
    ...
    for (int i = (M/STRIDE)*STRIDE; i <= M - 4; i += 4){
      for (int j = 0; j <= N - 4; j += 4){
          ...
          __m256d a_row_k_first_half;
          for (int k = 0; k < K; ++k){
            a_row_k_first_half = _mm256_loadu_pd(A+weird_offset_no_multiply(i, k, lda, STRIDE));
...
static void two_by_two_and_naive(double* A, double* B, double* C, int M, int N, int K, int lda){
  ...
  for (int i = (M/4)*4; i <= M-2; i+=2){
    for (int j = 0; j <= N-4; j += 4){
      double* Cij = C + i + j*lda;
      //load cols
      c_col_0 = _mm_loadu_pd(Cij);
      c_col_1 = _mm_loadu_pd(Cij + lda);
      c_col_2 = _mm_loadu_pd(Cij + 2 * lda);
      c_col_3 = _mm_loadu_pd(Cij + 3 * lda);

      __m128d a_row_k_first_half;
      int A_pos = weird_offset_no_multiply(i, 0, lda, STRIDE);
      for (int k = 0; k < K; ++k){
        a_row_k_first_half = _mm_loadu_pd(A + A_pos + k * STRIDE);
        ...
        _mm_storeu_pd(Cij,         c_col_0);
        _mm_storeu_pd(Cij+lda,     c_col_1);
        _mm_storeu_pd(Cij+2*lda,   c_col_2);
        _mm_storeu_pd(Cij+3*lda,   c_col_3);
      }
    }
    //leftover//
    for (int j = (N/4)*4; j < N; j++){
        c_col_0 = _mm_loadu_pd(C + i + j*lda);
        __m128d a_row_k_first_half;
        int A_pos = weird_offset_no_multiply(i,0,lda,STRIDE);
        for (int k = 0; k < K; k++){
          a_row_k_first_half  = _mm_loadu_pd(A + A_pos + k * STRIDE);
        ...
        _mm_storeu_pd(C+i+j*lda,      c_col_0);
      }
    }
